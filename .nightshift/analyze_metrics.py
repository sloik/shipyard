...
def compute_resolution_analytics(metrics):
    """Aggregate authoritative kickoff outcomes, collapsing rows by run_id."""
    runs = defaultdict(list)
    for row in metrics:
        resolution = row.get("resolution")
        if isinstance(resolution, dict) and resolution.get("run_id"):
            runs[resolution["run_id"]].append(resolution)

    gate_failures = 0
    unblock_attempted = 0
    automatic_unblock_successes = 0
    ...
    for rows in runs.values():
        ...
        if any(row.get("automatic_unblock_succeeded") is True for row in rows):
            automatic_unblock_successes += 1
        if any(row.get("later_session_required") is True for row in rows):
            deferred_recoveries += 1

        blocker = next(
            (
                row.get("blocker_class")
                for row in rows
                if row.get("blocker_class") not in (None, "none", "unknown")
            ),
            None,
        )
        if blocker:
    ...
    midpoint = len(final_latencies) // 2
    if not final_latencies:
        median_latency = 0.0
    elif len(final_latencies) % 2:
        median_latency = final_latencies[midpoint]
    else:
        median_latency = (
            final_latencies[midpoint - 1] + final_latencies[midpoint]
        ) / 2
    return {
        "runs": run_count,
        ...
        "automatic_unblock_success_rate": (
            ...
            if unblock_attempted
            else 0.0
        ),
        "deferred_recoveries": deferred_recoveries,
        "deferred_recovery_rate": (
...
def generate_markdown_report(
    ...
    if resolution_analytics and resolution_analytics.get("runs"):
        resolution = resolution_analytics
        lines.append("## Kickoff Resolution")
        lines.append("")
        lines.append(f"- **Linked runs:** {resolution['runs']}")
        lines.append(
            "- **Evidence-gate failure rate:** "
            f"{resolution['evidence_gate_failure_rate']:.1%}"
        )
        lines.append(
            "- **Automatic unblock success rate:** "
            f"{resolution['automatic_unblock_success_rate']:.1%}"
        )
        lines.append(
            "- **Deferred recovery rate:** "
            f"{resolution['deferred_recovery_rate']:.1%}"
        )
        lines.append(
            "- **Median resolution latency:** "
            f"{resolution['median_resolution_latency_s']:.0f}s"
        )
        if resolution["blocker_classes"]:
            counts = ", ".join(
                f"{name}={count}"
...
def generate_json_output(
    ...
    output = {
        ...
        "resolution_analytics": resolution_analytics or {},
    }
...
def main():
    ...
    resolution_analytics = compute_resolution_analytics(metrics)
