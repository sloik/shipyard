...
**Git policy:** Use `GIT.md` as the source of truth for commit format, merge
expectations, and reviewable git history.
...
**Commit format to expect:** `GIT.md` § Commit Format is authoritative. A typical
spec commit looks like:

```
[SPEC-001] feat: add search feature with fuzzy matching
