...
## Verification Report Severity Gate

Post-implementation review produces or consumes `.nightshift/reports/<spec-id>/verification.json` and `.md` (SPEC-052). Findings are grouped by dimension: completeness, correctness, coherence. Severity rules are binding:

- `CRITICAL` blocks `status: done` and requires a fix before finalization.
- `WARNING` requires a recorded rationale or linked follow-up task.
- `SUGGESTION` is non-blocking.

Every issue must include evidence and a concrete recommendation. Heuristic review must be labelled as heuristic; deterministic tests, drift checks, and validators remain the primary proof.
