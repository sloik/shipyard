...
**Git policy:** `GIT.md` is the source of truth for git workflow semantics.
...
Policy source: `GIT.md` § Baseline and Dirty Tree.
...
After the tooling audit, install the Nightshift git hooks:

```bash
# Pre-commit: lint + type-check before every commit
cp .nightshift/hooks/pre-commit .git/hooks/pre-commit
chmod +x .git/hooks/pre-commit

# Commit-msg: enforce ordinary spec and lifecycle commit subjects
cp .nightshift/hooks/commit-msg .git/hooks/commit-msg
chmod +x .git/hooks/commit-msg

# Branch guard (SPEC-118/119): keep the PRIMARY checkout on the default branch.
# Idempotent; chains onto the pre-commit + post-checkout installed above.
sh .nightshift/hooks/install-branch-guard.sh
# Then list this repo's primary toplevel in its registry so the guard activates:
#   echo "$(git rev-parse --show-toplevel)" >> .nightshift/protected-worktrees.txt
```
...
parent lifecycle transitions use `chore: mark SPEC-ID done|blocked`. Merge commits
and reverts are exempt. This keeps ordinary commits traceable and lets the installed
post-commit metrics hook observe valid lifecycle transitions. Disable by setting
`git.commit_prefix: "none"` in config.yaml.

**Verify after installation:** run `printf 'chore: mark SPEC-033 done\\n' > /tmp/nightshift-commit-msg && .git/hooks/commit-msg /tmp/nightshift-commit-msg`; it must exit 0. Change `done` to `in_progress`; it must exit non-zero.

**Why:** These hooks are agent-agnostic — they work regardless of which agent/harness runs the loop. The agent runs `git commit` → git runs the hooks → commit is rejected if lint fails or spec prefix is missing.
...
- [ ] Commit-msg hook is installed (`.git/hooks/commit-msg` exists and is executable)
