...
**When to read this:** If the spec's effective domain is NOT `code`, read this file before entering the loop. It maps each of the 16 steps to your domain (research or analysis) and explains what "passing" means for non-code work.

> **Per-spec domain resolution (v3.0):** The domain for each spec is resolved via cascade:
> `spec.domain → stack_profile.domain → config.runner.domain → "code"`.
> See LOOP.md § Domain Resolution for the full algorithm.
> This map defines *what changes* per domain; resolution defines *how the domain is determined*.
...
| 9.7. Synthesis gate | Usually skipped | Conditional handoff validation for direct code dependents | Conditional handoff validation for direct code dependents |
...
- **Step 9.7: Synthesis Gate** — Only for research/analysis specs with direct code dependents; validate downstream interface, then write `knowledge/handoffs/<spec-id>.json` and `knowledge/patterns/<spec-id>-findings.md`
- **Step 11: Outcome Routing (SPEC-042)** — Policy-driven retry/abort/escalate decisions; domain-agnostic
- **Step 11b: Capture TODOs** — Same knowledge capture process
