...
const failures = await withHarness(async ({ page, base }) => {
  ...
  if (onSwitch) {
    ...
    // Wait for it (it can paint a render-tick after the switch-off class flips)
    // rather than reading textContent the instant switch-off appears.
    let bannerShown = false;
    try {
      await page.waitForFunction(
        (sel) => ((document.querySelector(sel)?.textContent) || '').includes('Blocked by gateway policy'),
        ALPHA, { timeout: 5000 });
      bannerShown = true;
    } catch { /* banner never appeared */ }
    check('toggle off: alpha card shows "Blocked by gateway policy" banner', bannerShown);
    ...
    check('toggle on: alpha card no longer shows the blocked banner', bannerGone);
  }
