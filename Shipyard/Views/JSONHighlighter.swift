...
@MainActor final class JSONHighlighter {
    ...
    private static let punctuationColor = NSColor.labelColor.withAlphaComponent(0.6)
    
    /// Highlight a JSON string and return an NSAttributedString
    static func highlight(_ jsonString: String, fontSize: CGFloat = 11) -> NSAttributedString {
        let baseFont = NSFont.monospacedSystemFont(ofSize: fontSize, weight: .regular)
        let keyFont = NSFont.monospacedSystemFont(ofSize: fontSize, weight: .semibold)
    ...
    private static func parseString(_ json: String, startingAt start: String.Index) -> (Range<String.Index>, Bool)? {
        ...
        while index < json.endIndex {
            let char = json[index]
            if char == "\"" {
                let stringRange: Range<String.Index> = start..<json.index(after: index)
